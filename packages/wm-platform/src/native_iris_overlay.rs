...
use windows::{
  ...
  Win32::{
    ...
    Graphics::Gdi::{
      ...
      DeleteObject, EndPaint, GetDC, ReleaseDC, SelectObject,
      SetWindowRgn, UpdateWindow, HBITMAP, HGDIOBJ, HRGN, PAINTSTRUCT,
      RGN_DIFF, SRCCOPY,
    },
    UI::WindowsAndMessaging::{
      ...
      SWP_SHOWWINDOW, WM_ERASEBKGND, WM_PAINT, WS_EX_NOACTIVATE,
...
/// `GWLP_USERDATA`) to the window, and suppresses the default background
/// erase to avoid a flash. All other messages fall through to
/// `DefWindowProcW`.
    ...
    WM_PAINT => {
      ...
      if hdc.0 != 0 && raw != 0 {
        ...
        if mem_dc.0 != 0 {
          ...
          // SAFETY: both DCs are valid; the snapshot matches the client
          // size.
          let _ = unsafe {
...
fn capture_monitor(monitor: &Rect) -> crate::Result<HBITMAP> {
  ...
  // Compute the result without early-returning so the single `ReleaseDC`
  // below always runs. SAFETY: `screen_dc` is valid; every GDI object
  // created here is either freed here or handed back to the caller.
  let result = unsafe {
    ...
    if mem_dc.0 == 0 {
      ...
      let out = if bitmap.0 == 0 {
        ...
        // `monitor` coordinates share the screen DC's virtual-screen
        // space.
        let blit = BitBlt(
          mem_dc,
          0,
...
/// Topmost snapshot overlay that drives the iris-wipe workspace
/// transition.
///
/// On creation it captures the monitor's current content (the outgoing
/// workspace) into a frozen bitmap shown on a topmost, click-through
/// window fixed at the monitor rect. The caller then switches the real
/// windows underneath; each frame [`set_hole`](Self::set_hole) carves a
/// growing circular hole into the overlay so the live incoming workspace
/// shows through. Dropping the overlay tears down the window and frees the
/// snapshot.
///
/// The hole edge is hard (region-based); a soft/feathered edge would
/// require a per-pixel composite pass and is intentionally out of scope
/// here.
///
/// # Platform-specific
///
...
pub struct NativeIrisOverlay {
  ...
  hwnd: isize,
  /// Monitor rect; the overlay is fixed here and hole coordinates are
  /// mapped into its local space.
  monitor: Rect,
...
impl NativeIrisOverlay {
  /// Creates and shows the overlay covering `monitor`, frozen on a
  /// snapshot of whatever is currently composited there.
  ///
  /// The overlay starts fully covering (no hole). Call
  /// [`set_hole`](Self::set_hole) each frame to reveal the content
  /// beneath.
  ///
  /// Returns an error if the monitor cannot be captured or the window
  /// cannot be created; callers fall back to an instant (non-animated)
  /// switch.
  pub fn create(monitor: &Rect) -> crate::Result<Self> {
    ...
    // Stash the snapshot so the window procedure can blit it on
    // `WM_PAINT`. SAFETY: `hwnd` is valid; the stored value is an
    // opaque handle.
    unsafe {
      SetWindowLongPtrW(hwnd, GWLP_USERDATA, bitmap.0);
    }

    // Show the overlay topmost without activating it, then force an
    // immediate paint so the snapshot is on screen before the real
    // windows switch underneath (no blank frame).
  ...
  /// Carves a circular hole of `radius` pixels centered at screen
  /// coordinate `(cx, cy)` into the overlay, revealing whatever is
  /// beneath it.
  ///
  /// A non-positive `radius` shows the full snapshot (no hole). When the
  /// hole fully covers the monitor the overlay becomes empty (fully
  /// revealed).
  pub fn set_hole(&self, cx: i32, cy: i32, radius: i32) {
    let w = self.monitor.width();
    let h = self.monitor.height();

    // SAFETY: regions are created here; `SetWindowRgn` takes ownership of
    // the final region, and the temporary ellipse is deleted
    // explicitly.
    unsafe {
      ...
      // Ownership of `region` transfers to the window; do not free it
...
impl Drop for NativeIrisOverlay {
  fn drop(&mut self) {
    // SAFETY: both handles were created in `create`. Destroy the window
    // before freeing the bitmap it referenced.
    unsafe {
