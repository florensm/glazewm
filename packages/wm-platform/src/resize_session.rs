...
/// Upper bound on how long [`ResizeSession::pre_commit`] waits for the
...
/// called once the wait has already timed out, so the extra `OpenProcess`/
/// `QueryFullProcessImageNameW` cost never lands on the common (fast)
/// path.
fn process_name_for_warning(hwnd: HWND) -> Option<String> {
  ...
  // SAFETY: `process_handle` is valid; `buffer`/`length` are
  ...
  exe_path.rsplit('\\').next().map(|file_name| {
...
pub struct SessionOptions {
  ...
  /// Color/width/corner-radius/opacity for the border overlay tracking
  /// this session, or `None` when the border effect isn't configured.
  /// Same snapshot rationale as `blur_overlay`.
...
pub struct ResizeSession {
  ...
  ///
  /// The blur overlay tracks this session's surrogate at a fixed
  ...
  /// frame while the window it's supposedly attached to is still a
  /// speck at the center. Scaling the overlay's own opacity by this
  /// value in [`blur_overlay_params`] instead fades it in/out in
  /// lockstep with the thumbnail's zoom.
  ///
  ...
  /// `GetWindowRect` poll -- kept separate rather than shared, since the
  /// two polls run in different (though normally non-overlapping)
  ...
  /// Color/width/corner-radius/opacity for the border overlay tracking
  /// this session, snapshotted from `SessionOptions::border_overlay`.
  /// See [`border_overlay_params`].
  ///
  ...
  /// `true` once [`pre_commit`] has confirmed (or given up waiting for,
  /// see [`COMMIT_CONFIRM_MAX_WAIT`]) the real window landing at
  /// `target_rect`. The caller must not uncloak/fade the surrogate
  /// until this is `true`. Reset to `false` by [`update_target`] on a
  /// redirect, since that invalidates whatever commit was in flight for
  /// the old target.
  ///
  /// [`pre_commit`]: ResizeSession::pre_commit
  /// [`update_target`]: ResizeSession::update_target
  commit_confirmed: bool,
  /// When `pre_commit`'s asynchronous move was issued, or `None` before
...
impl ResizeSession {
  ...
  /// the sampled backdrop color until then. When surrogate
  /// creation/revival fails the animation falls back to direct
  /// repositioning.
  ///
  /// [`begin`]: ResizeSession::begin
  /// [`begin_reusing_surrogate`]: ResizeSession::begin_reusing_surrogate
  fn begin_impl(
    ...
    // `ACCENT_ENABLE_GRADIENT`, with the (possibly gap-having,
    // mid-resize) thumbnail composited on top -- so it's the fill
    // behind any area the thumbnail doesn't yet cover, not just a
    // border strip.
    //
    ...
    //
    // - Applied only when the target exceeds the source on some axis. Pure
    //   moves and pure shrinks never uncover anything (the thumbnail is
    ...
    // cached color -- this never samples inline. The two-`BitBlt`
    // GPU->CPU readback used to run synchronously right here, stalling
    // the WM's single main thread for tens of milliseconds per window
    // on the first resize of a burst (measured: 26-114ms per call).
    // Callers now warm the cache in the background instead -- see
    // `sample_edge_color_async` -- so a cache miss just means one
    // animation plays with a transparent gap instead of blocking the
    // keypress that started it. Two different things, deliberately:
    // what the *surrogate* paints, and what the session remembers.
    //
  ...
  /// (the real window is cloaked for the duration), so the overlay must
  /// sit directly behind *it*, not the (hidden) real window.
  #[must_use]
  ...
  ///
  /// Needed by the fade-out tail: the real window is uncloaked *beneath*
  /// the still-visible surrogate and the surrogate then fades over it,
  /// so for that stretch the window -- not the surrogate -- is what an
  /// overlay has to sit behind. Anchoring to the surrogate there wedges
  /// the overlay between the two, and an opaque overlay hides the window
  /// it belongs to for the whole fade.
  #[must_use]
  ...
  /// acrylic-overlay tracker to follow this tick, or `None` when there's
  /// nothing to show.
  ///
  ...
  /// `target_rect` for the whole animation (see [`update_zoom_fade`]) --
  /// so this always reports the fixed target rect for them; no per-frame
  /// storage is needed.
  ///
  /// [`update_zoom_fade`]: ResizeSession::update_zoom_fade
  #[must_use]
  ...
  pub fn update_zoom_fade(&mut self, progress: f32, opacity: u8) {
    ...
    if half_w <= 0 || half_h <= 0 {
      ...
      // `update_thumbnail_dims` -- otherwise every zoom-fade frame samples
      // a few pixels up-left of the true content area.
      // `rcDestination` is destination-space within the surrogate's
      // own client area (already sized to the logical rect), so it
  ...
  pub fn update_clipped(
    ...
    // `rcSource` samples from the real window's coordinate space, so it
    // must carry the same `border_inset` offset as
    // `register_thumbnail`/ `update_thumbnail_dims` (see
    // `update_zoom_fade`'s comment) -- `src_left`/`src_top` above are
    // the visible slice's offset within the *logical* rect, not yet
    // shifted into the window's physical/source space.
    surrogate.set_thumbnail_rects(
      RECT {
  ...
  ///   (`maybe_handoff`/the initial async preposition normally got it
  ///   there already), finishes immediately. Otherwise issues an
  ///   *asynchronous* move (`SWP_ASYNCWINDOWPOS`) and returns `false` --
  ///   unlike a plain `SetWindowPos`, this never blocks the calling thread
  ///   on the target process's message queue, however slow it is.
  /// - Later calls: polls (throttled, see `commit_poll_parity`'s doc
  ///   comment) for the real window to have actually reached
  ///   `target_rect`, finishing once it has. If
  ///   [`COMMIT_CONFIRM_MAX_WAIT`] elapses without that happening (a hung
  ///   app, or one that's genuinely this slow), gives up and finishes
  ///   anyway -- seeing the caller uncloak a window that's not quite at
  ///   its final rect yet is a much smaller cost than the unbounded
  ///   main-loop stall this replaced.
  ///
...
/// main thread; wrapped in `Arc<Mutex<_>>` so
/// [`sample_edge_color_async`]'s background thread can insert into the
/// same map the main thread reads from, without routing the result back
...
/// `AnimationManager`'s
/// `EDGE_COLOR_CACHE_PRUNE_LEN`/`EDGE_COLOR_CACHE_TTL`) -- pruning happens
/// here, on the background thread, rather than on the caller's next
/// synchronous insert.
