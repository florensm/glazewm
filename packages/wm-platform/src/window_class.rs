...
/// which each need a distinct class name and (for the iris overlay) window
/// procedure, but otherwise register identically -- previously each
/// copy-pasted its own `OnceLock` + `WNDCLASSW` + `RegisterClassW` call.
///
...
/// window disappearing and being replaced by its own blur.
