...
pub struct AnimationManager {
  ...
  /// slot, not just the surrogate's own window. `sync_blur_overlays`
  /// checks this to fall back to a full per-window z-order resync for
  /// one tick instead of its normal narrowly-targeted one (see its
  /// `z_order_touched` parameter), then clears it. Not cleared on its
  /// own -- callers that create such a surrogate are responsible for
  /// setting it, `sync_blur_overlays` is responsible for resetting it
  /// after acting on it.
  #[cfg(target_os = "windows")]
...
impl AnimationManager {
  ...
  /// overlay while the real window is cloaked — preventing the overlay
  /// from jumping to the window's pre-positioned target rect while the
  /// surrogate is still mid-animation.
  #[cfg(target_os = "windows")]
  ...
  pub(crate) fn update_internal(
    ...
    #[cfg(target_os = "windows")]
    {
      ...
      // Batches every closing window's acrylic-overlay reposition this
      // tick into one `DeferWindowPos` transaction instead of each
      // issuing its own synchronous `SetWindowPos` -- same reasoning
      // as `platform_sync`'s per-tick blur-overlay sync passes.
      ...
      for id in &close_in_progress {
        ...
        // the acrylic blur and border overlays directly here
        // instead, mirroring `sync_blur_overlays`'/
        // `sync_border_overlays`' steady-state behavior but
    ...
    #[cfg(target_os = "windows")]
    {
      ...
      for id in close_done {
        ...
        // `upsert_blur_overlay`/`upsert_border_overlay` since closing
        // windows are detached from the layout tree and never
        // reach `sync_blur_overlays`/`sync_border_overlays`'s
        // normal retain-based cleanup. Remove them here too, or
        // the overlay window they own is never destroyed and
    ...
    let ws_complete_ids: Option<Vec<Uuid>> = {
      ...
      // the live blur-overlay tracker to pick `focused_window` vs
      ...
      if let Some(ws) = &mut state.animation_manager.workspace_switch {
        ...
        for (&window_id, entry) in ws.windows.iter_mut() {
          if let Some(ref mut s) = entry.surrogate {
            ...
            // same acrylic blur overlay used in steady state
            // is kept alive and repositioned to follow the
            ...
            if s.is_live() {
              match s.current_rect() {
                ...
                None => {
                  if let Some(overlay) =
                    state.blur_overlays.get_mut(&window_id)
                  {
                    overlay.hide();
                  }
                }
            ...
            // border tracks every surrogate regardless of acrylic
            // backdrop.
            {
    ...
    #[cfg(target_os = "windows")]
    {
      ...
      // real-window + blur-overlay composite.
      ...
      // Batched into one `DeferWindowPos` transaction, same
      // reasoning as the other blur-overlay sync loops.
      ...
      if !finishing_sessions.is_empty() {
        ...
        for (id, rect) in &finishing_sessions {
          ...
          if let Some(overlay) = state.blur_overlays.get_mut(id) {
            ...
            tracing::debug!(
              "finishing_sessions: no blur overlay for {id}."
            );
          }
          if let Some(overlay) = state.border_overlays.get_mut(id) {
      ...
      // `sync_blur_overlays` (called from `platform_sync` above) hides
      // each window's static acrylic overlay while its
      // workspace-switch surrogate is alive, including this
      // `pending_ws_cleanup` grace period. The surrogate itself
      // carries a live SWCA acrylic blur throughout, so that's fine
      // while it's alive -- but dropping `pending_ws_cleanup`
      // below destroys the surrogate outright, and nothing shows the
      // static overlay again until a later `platform_sync` call
      // runs. Between those two moments DWM can composite a frame
      // with *no* blur behind the (semi-transparent) real window at
      // all -- a one-frame flicker at the end of every switch.
      // Pre-show the static overlay at its final rect
      ...
      // total_border_delta()`, including the OS's invisible
      // drop-shadow expansion) meant for `SetWindowPos`, not overlay
      // tracking. Using `surrogate.rect` directly here made
      // blur/border overlays pop visibly larger right at the end of
      // every workspace switch, then snap down once a later
        ...
        if let Some(tint) = effect_cfg.backdrop.overlay_tint() {
          ...
          match state.blur_overlays.entry(*id) {
            ...
            std::collections::hash_map::Entry::Vacant(e) => {
              if let Ok(overlay) =
                NativeBlurOverlay::create(rect, params, *anchor)
              {
                e.insert(overlay);
              }
            }
      ...
      // `sync_blur_overlays` path. DWM can composite a frame in that
      // gap showing the overlay in whatever stale z-slot that leaves
      // it -- a one-frame flicker at the end of every switch, on
      // every incoming window (masked on the focused one
  ...
  pub fn start_animation_if_needed(
    ...
    #[cfg(target_os = "windows")] blur_overlay: Option<BlurOverlayParams>,
  ...
  /// only after it has also queued the blur/border overlays that ride
  /// along with these surrogates, so window and overlays land in a
  /// single `DeferWindowPos` transaction: sequential transactions can
  /// straddle a composition boundary and let a surrogate move a frame
  /// ahead of its own border outline, or adjacent windows' edges visibly
  /// desync.
  ///
  ...
  pub fn start_open_animation(
    ...
    match ResizeSession::begin(
      ...
      Ok(mut session) => {
        ...
        // correct progress. `place_at_top: true` above means the
        // session's surrogate (if any) was inserted at `HWND_TOP`
        // -- see `blur_overlay_z_order_dirty`'s doc comment.
        if session.surrogate_hwnd().is_some() {
          self.blur_overlay_z_order_dirty = true;
        }
