...
pub fn unmanage_window(
  ...
  // Destroy any static backdrop/border overlay owned by this window
  // directly, rather than leaving it for `sync_overlays`'s retain-based
  // cleanup on the next `platform_sync`. That cleanup only runs when
  // `pending_sync` has changes, but `focus_target_after_removal` returns
  // `None` -- queuing no change -- whenever the unmanaged window wasn't
  // focused, which is common for a floating window that was closed,
  // hidden, or found invalid in the background. Without this, the overlay
  // window it owns is never destroyed and lingers on screen as a ghost
  // until some unrelated later sync happens to run (mirrors the equivalent
  // cleanup for completed close animations in
  // `AnimationManager::update_internal`).
  #[cfg(target_os = "windows")]
  {
    state.backdrop_overlays.remove(&window.id());
