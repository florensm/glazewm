...
#[cfg(target_os = "windows")]
use wm_common::{
  OverlayTracking, WindowEffectConfig, WorkspaceSwitchStyle,
};
#[cfg(target_os = "windows")]
...
fn redraw_containers(
  ...
  for window in windows_to_update.iter().rev() {
    ...
    let (effect_opacity, corner_style, blur_overlay, border_overlay) = {
      ...
      let border_overlay =
        effect_cfg.border.abgr_color().filter(|_| is_tracked).map(
          |color| {
...
fn sync_overlays<O: SyncableOverlay>(
  ...
  for window in &all_windows {
    ...
    let Some(params) = (if is_focused {
      focused_params
    } else {
      other_params
    }) else {
      // Configured, but not for this window's focus state: hide rather
      // than destroy. Guarded on `is_visible` because `hide` issues
      // a `ShowWindow` unconditionally.
