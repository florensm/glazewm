...
fn sync_tab_bars(state: &mut WmState, config: &UserConfig) {
  ...
  // Pixel conversion uses a scale of 1.0; per-stack DPI scaling is already
  // applied by `tab_bar_height_px()` on the `StackContainer`. The additional
  // style lengths (indicator, radius, separator) follow the same convention.
