...
pub struct PendingSync {
  ...
  /// Window IDs whose fullscreen state was changed by a WM command this
  /// sync cycle (`set-fullscreen` / `toggle-fullscreen`), as opposed to
  /// the app driving itself fullscreen.
  ///
  ...
  /// mode-set the display, and cloaking one behind a surrogate kicks it
  /// out of fullscreen, reverts the resolution and re-triggers the
  /// relayout in a loop. An app driving itself fullscreen is detected
  /// in `handle_window_moved_or_resized` and routed through
  ...
  ///
  /// `platform_sync` plays the `window_minimize` transition inwards for
  /// these -- the same entry animation `window_open` uses for a
  /// brand-new window. A restore cannot be inferred from
  /// `previous_target.is_none()` the way an open can, because a
  /// restored window kept the target position it had before it was
  /// minimized.
